...
def measure_delay( args, rate,
    ...
    try:
        ...
        print(f"ca marche pas: attente freq {freq}")
        c = sys.stdin.read(1)
    print("setFrequency ok")
...
def main():
    ...
    parser.add_argument("--rate", type=float, help="Tx and Rx sample rate", default=4e6)
    ...
    #for freq in range(int(30e6), int(3000e6), int(4e6)):
    if True:
        #options.freq = int(freq)
        measure_delay(
            args=options.args,
            rate=options.rate,
            freq=options.freq,
...
#MeasureDelay.py --rate=2e6 --freq=96e6 --clock-rate=45e6
